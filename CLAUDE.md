...
- **Import automation**: `scripts/install_cron.sh` installs a *monthly* job (20th, 08:00 — reports arrive mid-month). `run_import.sh` snapshots the DB to `data/backups/` (last 10) before each run; `run_import.py` exits 5 ("already ingested") when the newest report in the mailbox is one `import_log` already recorded — the fetcher always returns the newest match and has no memory, so a late report would otherwise re-import as a misleading "0 new rows". It emails a summary on every outcome via `src/input/notifier.py` (Gmail SMTP, same app password as IMAP) and skips the ~8-minute rebuild when no rows were inserted.
